...
async function createComicFormat(token, imageIDs, comicID) {
  const response = await fetch(
    settings.BACKEND +
      "/page/imageinboxes?comicID=" +
      comicID +
      "&imageIDs=" +
      JSON.stringify(imageIDs.filter((id) => id !== "")),
    {
      method: "GET",
      headers: {
...
export default function CreatePage() {
  ...
  const onSubmit = async (event) => {
    ...
    try {
      const response = await fetch(settings.BACKEND + "/page/create-page", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      createComicFormat(token, imageIDs, comicId).then((response) => {
        console.log(response);
      });

      //Redirect to the comic page react
      setCreated(true);
    } catch (error) {
      console.error(error);
    }
  ...
  if (created) {
    return <Navigate to={"/viewComic?id=" + comicId} />;
  }
  return (
